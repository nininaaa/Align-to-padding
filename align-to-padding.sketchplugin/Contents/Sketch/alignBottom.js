...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/alignBottom.js");
/******/ })
/************************************************************************/
/******/ ({
  ...
  if (run.complete === true) {
    ...
    selectedLayers.forEach(function (item) {
      if (run.lastLayer.frame.y > item.frame.y) {
        item.frame.y = lastLayerHeight - (item.frame.y - run.lastLayer.frame.y) - item.frame.height - Number(run.padding[2]);
      } else {
        item.frame.y = lastLayerHeight - item.frame.height - Number(run.padding[2]);
      }
    });
