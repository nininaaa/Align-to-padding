...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/alignRight.js");
/******/ })
/************************************************************************/
/******/ ({
  ...
  if (run.complete === true) {
    ...
    selectedLayers.forEach(function (item) {
      if (run.lastLayer.frame.x > item.frame.x) {
        item.frame.x = lastLayerWidth - (item.frame.x - run.lastLayer.frame.x) - item.frame.width - Number(run.padding[1]);
      } else {
        item.frame.x = lastLayerWidth - item.frame.width - Number(run.padding[1]);
      }
    });
